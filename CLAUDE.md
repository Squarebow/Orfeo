...
Enabled via Settings → MIDI Files & Library → Note Editor (eye toggle). Activated via the pencil icon in TopBar (visible only when `noteEditorEnabled && midi`). Entry point in `src/components/Transport/TopBar.tsx`.

**`src/utils/noteEditorState.ts` — NES singleton:**
- Bridges `NoteEditorToolbar` ↔ `PianoRoll` without prop threading.
- `NES.editMidi` — a proper `@tonejs/midi Midi` instance, the ONLY correct edit target. Created in `PianoRoll.tsx` drawFrame on mode enter via `midiToEditableCopy((parsedMidi as any)._raw)`. Null when not in edit mode.
- `NES.reset()` — called on mode enter/exit; clears `editMidi`, history, dirty, newNotes, tool state.

**Critical: `ParsedMidi` vs `@tonejs/midi Midi`:**
- `storeRef.current.midi` is a `ParsedMidi` — plain copied objects (`ParsedNote[]`). No `ticks`, no `durationTicks`, no `addNote()`, no `header`. **Never use it as the edit target.**
- `NES.editMidi` is a real `@tonejs/midi Midi` — live Track/Note objects with `ticks`, `durationTicks`, `addNote()`, and a working `header`. **Always route edit ops through `NES.editMidi.tracks`.**
- `ParsedMidi.tracks[i]` ↔ non-empty `NES.editMidi.tracks[i]` by position. Filter `editMidi.tracks` to `.filter(t => t.notes.length > 0)` first — mirrors `parseMidiBuffer`'s empty-track skip. Use `parsedMidi.tracks[i].index` as the `trackIndex` for store lookup.

**`buildEditFlatNotes()`** in PianoRoll closure:
- Iterates `NES.editMidi.tracks`; maps `editMidi.tracks[i]` → `parsedMidi.tracks[i].index` for color/visibility.
- `EditFlatNote.trackIndex` = parsedTrack.index (used for channel lookup, not array position).

**`syncNoteTimes(note)`** — always use `(NES.editMidi as any)?.header`. ParsedMidi has no `header` — calling `syncNoteTimes` without `NES.editMidi` is a silent no-op.

**`snapTick()`** — uses `(NES.editMidi as any)?.header?.ppq`.

**Edit mode enter/exit in `drawFrame`:**
```
if (noteEditorActive && !prevNoteEditorActive) → create NES.editMidi, set needsFlatRebuild
if (!noteEditorActive && prevNoteEditorActive) → null NES.editMidi, null lastMidiRef (restore ParsedMidi display)
```

**`NES.needsFlatRebuild`** → rebuilds `flatNotesRef` directly from `NES.editMidi.tracks` (does NOT use `lastMidiRef = null` trick in edit mode).

**Wheel scrub blocked during drag only** via `editDragActiveRef: useRef(false)` — a component-level ref shared between the wheel-handler effect and the PixiJS closure. Set true on editDrag init, false on editDrag null.

**Audio preview** — `__orfeoPlayNote(midi, vel, dur, channel?)` accepts a MIDI channel. Edit handlers pass `getTrackChannel(ef.trackIndex)` so notes preview on the correct instrument. Both GM and Samples engines support the channel param.

**Pre-play instrument audio — two-engine fix:**

- **Samples engine (`useSamplesEngine.ts`):** SpessaSynth defaults all channels to program 0 (piano). `buildSamplesPlayer` is the only place `_synth.programChange()` was called — only triggered on playback start. Fix: module-level `_samplesChanInit: Set<number>`. In `__orfeoPlayNoteSamples`, first use of any non-preview channel (ch ≠ 15) triggers a `programChange(ch, track.program)` lookup from the store. `buildSamplesPlayer` clears and repopulates this set as part of its own program-change loop, so after playback the set tracks what SpessaSynth actually has.
- **GM engine (`useAudioEngine.ts`):** `_port.send([0xC0|ch, program])` is silently discarded before any JZZ player has been connected to `_port`. The synthesizer's internal pipeline is only activated by `player.connect(port)` inside `buildPlayer`. Fix: `warmupEditChannels()` creates a temporary JZZ SMF player connected to `_port` with a filter that passes program changes / bank-select CCs only (blocks all notes). Plays for 150 ms then stops. `_warmupPromise` stores the result; `playNote` awaits it if a click arrives during the window. Called via `window.__orfeoWarmupEditChannels` on edit mode enter. `_warmupPromise` cleared in `destroyPlayer` so re-entry after a file change re-warms.

**Non-empty-track filter required in ALL four edit paths:** `parseMidiBuffer` skips `track.notes.length === 0` tracks; `midiToEditableCopy` does not. Any position-based mapping of `editMidi.tracks[i] ↔ parsedMidi.tracks[i]` must filter to non-empty first. Four locations in PianoRoll.tsx require this: `buildEditFlatNotes`, `needsFlatRebuild` block, alt+click `editTrack` lookup (`nonEmptyForAdd`), and `getTrackChannel`. Missing the filter in any one location causes wrong track assignments and wholesale color/channel reshuffling on rebuild.

**`selection-move` drag axis:** starts `null`, determined by first gesture exceeding `DRAG_THRESHOLD`. Horizontal drag → `'pitch'` (shifts all selected notes by same delta from `origMidi`). Vertical drag → `'time'`. `selectionSnapshot` items include `origMidi: number`. Both axes have history entries. Do NOT default `axis: 'time'` at drag init — that locks out horizontal drag entirely.

**`history.push(cmd)` does NOT call `apply()`** — caller must call `cmd.apply()` first, then `NES.history.push(cmd)`.

**Save flow:** `NoteEditorToolbar.handleSave` → `saveFileDialog` (versioned `_ORFEO` suffix) → `editableCopyToBuffer` → `noteEditor:save` IPC → reload file into store. `NES.onSaveRequest` registered on mount so `SettingsPanel` and app-close handler can trigger save without circular imports. Unsaved-changes guards implemented in `SettingsPanel.handleLoadFile` and `App.tsx` `app:save-before-close` listener.

---

## Presentation Mode Architecture

Distraction-free OS-level fullscreen for live playing and screen recording. Toggle: **F11** or the amber Expand/Shrink icon in the footer far-right corner. **Esc** exits.

**State:** `presentationMode: boolean` in the Zustand store (not persisted — always starts `false`). Setter: `setPresentationMode`.

**OS fullscreen:** `window.electronAPI.setFullScreen(value)` → IPC `window:setFullScreen` → `mainWin.setFullScreen(value)`. Three-file requirement: `electron/main.ts` + `electron/preload.ts` + `src/types/index.ts`.

**Enter flow with Note Editor guard (`enterPresentationMode` in `App.tsx`):**
1. If `noteEditorActive && NES.dirty` → show Save/Discard/Cancel dialog (mirrors `SettingsPanel.handleLoadFile`).
2. On Cancel → abort. On Save → await `NES.onSaveRequest()`, abort if fails. On Discard → clear `NES.dirty`.
3. Call `setPresentationMode(true)`.
Side effects in `useEffect([presentationMode])`: calls `setFullScreen`, hides header, calls `setNoteEditorActive(false)` (PianoRoll's existing `drawFrame` cleanup runs automatically).

**Header hover-reveal:** In PM, `<TopBar />` is removed from the flex column and re-rendered inside a `position: fixed, zIndex: 10000` wrapper with `transform: translateY(-100%)` (hidden) / `translateY(0)` (visible) + `transition: transform 0.25s ease`. An 8px invisible strip at `top: 0, zIndex: 10001` is the hover zone; `pointerEvents: none` when header is visible so the TopBar handles its own events. Hover zone `onMouseEnter` reveals immediately; TopBar `onMouseLeave` triggers 400ms debounce hide via `hideHeaderTimerRef`.

**What's hidden in PM (vs. normal):**
- `<SettingsPanel />` and `<TrackPanel />` — not rendered at all (PianoRoll expands to fill width naturally).
- Separator line between TopBar and content — not rendered.
- Chord bar in `Keyboard.tsx` — `{!presentationMode && <div ...>}` collapses to zero height.
- In `KeyboardControls.tsx`: Dock/Float button, flex-1 spacer, hand-label visualization, note counter.

**What stays in PM:**
- PianoRoll waterfall (fills full screen width).
- `<Keyboard />` (docked; floating mode left as-is if user had it set).
- Key-range selector (61/73/88) — always rendered regardless of `performanceHideControls`.
- PM toggle icon (Expand/Shrink) — `marginLeft: 'auto'` pushes it to the true right edge.

**Keyboard shortcuts:** `F11` toggles (enter uses `enterPresentationMode` for dirty check; exit calls `setPresentationMode(false)` directly). `Escape` exits PM before all other Escape handlers in `handleKeyDown`.

**Hand-label visualization in footer:** Intentionally hidden in PM (`{showHandLabels && !presentationMode && ...}`) — the feature is beta and needs rework before it should be visible in PM.

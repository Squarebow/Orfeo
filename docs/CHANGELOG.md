...
### New
- **Auto chord-tracking mode, now the default.** Reads the chord chart the way a musician would — from the instruments that actually play chords (piano, keys, guitar, strings, ensembles), ignoring melody and ornament lines whatever their instrument. It matches beat-synchronised pitch-class analysis against a set of everyday chord names, so an arpeggio, a broken chord and a block chord all read as the same name, changes land on the beat, and the bass line anchors the root (and fills in when a song's harmony *is* an arpeggiated bassline). Even a few bars with no chord instrument playing get named from the surrounding harmony rather than left blank.

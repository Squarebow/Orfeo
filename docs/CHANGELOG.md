...
- **Uninstall "delete everything" now actually clears your settings.** The per-machine installer's uninstaller runs elevated with the all-users shell context, where `$APPDATA` points at `C:\ProgramData` — so the cleanup step was deleting a folder that doesn't exist and your real `orfeo-prefs.json` (with the library-folder link) survived, making a fresh reinstall reopen straight into the old library. The cleanup now forces the invoking user's context, deletes the prefs/log files by name first (so a locked cache subfolder can't leave the library link behind), and, if the prefs file somehow still exists afterward, says so with the exact path instead of failing silently. It also never prompts or deletes anything during an auto-update (`--updated`) or a silent (`/S`) uninstall — an update keeps your library and files.

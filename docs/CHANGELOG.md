...
- **"Reflect piano roll on keyboard"** (Settings → Keyboard → Testing). While paused or scrubbing, the on-screen keyboard continuously lights whatever notes are sounding at the playhead, straight from whichever tracks are flagged "Lit on keyboard" in the Mixer or Track panel — no more right-clicking a chord and choosing "Show on keyboard" just to see what's under the cursor while scrubbing through a passage. Off by default; real playback lighting is untouched either way.

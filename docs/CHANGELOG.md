...
### New
- **Auto chord-tracking mode, now the default.** Reads the chord chart the way a musician would — from the instruments that actually play chords (piano, keys, guitar, strings, ensembles), ignoring melody and ornament lines whatever their instrument. Beat-synchronised pitch-class analysis first fixes a plain triad per beat (so an arpeggio, a broken chord and a block chord all read as the same name and changes land on the beat), then adds sevenths and extensions back per chord only where the colour tone is held the whole way through — a melody note brushing the 9th no longer flips the chord to an add9. The bass line is consulted only for slash chords, never to choose the root or quality.
...
### Changed
- **"Harmony" is now "Auto over every track".** Same detector, but it pools all non-drum tracks (melody included) instead of just the chord instruments — for dense arrangements where no single instrument carries the harmony. **"Follow"** stays a manual scope to one instrument or group.

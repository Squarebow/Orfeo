...
### Fixed
- **A chord could display up to three quarters of a second before you actually heard it**, when a brief chord just before it happened to die out in the same beat the next chord's own display was reading from. The display now anchors to the exact moment the new chord's own notes are struck together, not just the nearest beat line.
- **Volume dropped after resuming playback or seeking to a new spot mid-song** (GM sound engine only). Seeking sends a MIDI "reset all controllers" message that was quietly cancelling out the volume level right after it was set; volume is now re-applied after the seek instead of before, so it holds.
- **"Follow" chord tracking with no instrument or group chosen yet was silently reading every track in the file, bass included**, instead of falling back to the same smart default Auto mode uses — this is what made Follow mode look like it was "reading the bass" on some songs.
- **Locked or right-click-shown chords on the on-screen keyboard now light up in their own color** (a soft pink) instead of amber, so they're never confused with "Reflect piano roll on keyboard"'s amber reading when both apply to the same notes at once.
- **Right-clicking a chord and choosing "Show on keyboard" showed a generic, rebuilt voicing instead of the chord exactly as it's played.** The real notes at the playhead were already being read correctly, then discarded in favor of a synthesized one-octave shape — so a chord's actual register, inversion, and any doubled notes never matched what's on the piano roll. It now shows the real voicing, straight from the file. ("Open in Chord Explorer" is unaffected — it still gets the clean, canonical shape, which is what that view is for.) As a side effect, this also fixes a doubling bug with "Reflect piano roll on keyboard" (Settings → Keyboard): right-clicking to lock a chord while that's on used to add a second, differently-voiced copy of the same chord instead of matching what was already lit.
- **Isolating a track to review its chords — hiding everything else on the piano roll — didn't show anything on the keyboard.** Both "Reflect piano roll on keyboard" and right-click "Show on keyboard" were reading which tracks are flagged to light up during real playback (piano/keys by default) instead of which tracks are actually shown on the piano roll. Now they always match whatever you've got visible — hide down to one guitar or string part and its notes light the keyboard, exactly as advertised.
...
- **Downloading or importing a soundfont didn't switch to it.** After a FluidR3 / MuseScore download finished, or after importing your own .sf2, you still had to open the dropdown and pick it by hand. It's now selected and loaded automatically — the dropdown is just for switching between soundfonts you've already got.
- **A hidden track could still light up the on-screen keyboard during playback, in its own color, for no visible reason.** Real-time keyboard lighting only checked a track's "Lit on keyboard" switch, not whether it was actually shown on the piano roll — so a track you'd hidden could still light the keys during playback with nothing on screen to explain it. Now it only lights up while both switches agree, matching how pausing/scrubbing and right-click "Show on keyboard" already behave.
- **The chord name and keyboard lights could run slightly ahead of the sound itself.** Every sound device has some real delay between "play this note" and it actually reaching the speakers; nothing in the app accounted for it, so the display always updated a touch early. It now reads that delay from your actual audio setup and holds the display back to match — measured at about 40ms on a typical setup. How much this fully resolves "not quite on the beat" depends on your own hardware; a further pass may still be needed once real-world testing confirms how far it closes the gap.
...
### New
- **A second, opt-in way to read chords — "Chord reading" (Settings → Notation & Chords), Safe or Progressive.** Safe is the existing reading and stays the default, completely unchanged. Progressive also trusts a real, clean grab of notes — several played together, or rolled quickly within about an octave — to show a brief chord's extra color (like a 7th) right away, even when it doesn't ring long enough for Safe's own check to trust it. Switches instantly on a song that's already open, no reload needed.
- **A measured output-delay readout next to the Sound engine setting** (Settings → Audio) — shows the real gap between telling your sound card to play a note and it actually reaching the speakers, since every machine answers this differently.
- **Auto chord-tracking mode, now the default.** Reads the chord chart the way a musician would. It listens to the instruments that actually play chords (piano, keys, guitar, strings, ensembles), sets the melodic fills aside so the tune never changes the chord, and marks a new chord only where the accompaniment genuinely turns over — so a held chord under a busy melody stays put, an arpeggio is gathered into one name, and three real changes in a bar show as three chords. Each span is named against the full vocabulary, with a lean toward the plainer name so a passing note can't turn a triad into a 13th. The bass line adds a slash / inversion, nothing more.
...
- **Chord sensitivity slider** (Settings → Notation & Chords). One control for how finely the detector splits the beat — left for the underlying harmony, right to catch passing and embellishing chords. It applies to every tracking mode, so the mode now only chooses which tracks are read and the slider chooses the detail.
- **Progression playback in both Explorers is voiced like a player, not triggered.** A new **Voicing** toggle — Roots (root position, stepping up), Climbing (first chord low, then the nearest inversion just above each time — always ascending, never resetting to the octave), Smooth (the nearest voicing to the one before, fewest notes moving) — replaces the old off / sequential / random inversion buttons, and is shared between the Chord and Scale Explorers. Each chord is lightly rolled bottom-up with a little velocity variation. The separate Style dropdown still sets the chord quality per degree.
- **"Reflect piano roll on keyboard"** (Settings → Keyboard). While paused or scrubbing, the on-screen keyboard continuously lights whatever notes are sounding at the playhead, straight from whichever tracks are currently visible on the piano roll — no more right-clicking a chord and choosing "Show on keyboard" just to see what's under the cursor while scrubbing through a passage. Off by default; real playback lighting is untouched either way.
- **"Hide all" button on the Tracks panel.** Same eye icon and idea as the Mixer's mute-all — clears every track off the piano roll in one click so you can bring individual ones back manually while reviewing a song. Matches the same toggle already in the Console Mixer, so both stay in sync.

### Changed
- **"Harmony" and "Follow" are scope choices only now.** "Harmony" pools every non-drum track (melody included) for dense arrangements where no one instrument holds the chords; "Follow" scopes to one instrument or group you pick. How much detail each surfaces is the new Chord sensitivity slider, not the mode — the old fixed per-mode sensitivity is gone.
...
- **The chord sensitivity slider is temporarily switched off** (dimmed in Settings, reads at a fixed level) while the underlying chord-reading algorithm itself is still being tuned — a moving target on the detail level made it unclear whether a wrong reading came from the algorithm or from wherever the slider happened to be sitting. Nothing is deleted; it'll come back once the algorithm is settled.
- `src/App.tsx`, `src/components/ChordExplorer.tsx`, `src/components/ContextMenu.tsx`, `src/components/Keyboard/Keyboard.tsx`, `src/components/ScaleExplorer.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/Transport/TopBar.tsx`, `src/hooks/useAudioEngine.ts`, `src/hooks/useChordSequence.ts`, `src/hooks/useSamplesEngine.ts`, `src/store/index.ts`, `src/types/index.ts`, `src/utils/beatGrid.ts` (new), `src/utils/beatGridTest.ts` (new), `src/utils/chordDetection.ts`, `src/utils/chordSequenceBuilder.ts` (new), `src/utils/chordSequenceBuilderTest.ts` (new), `src/utils/chordVocabulary.ts` (new), `src/utils/chordVocabularyTest.ts` (new), `src/utils/debugTimestamp.ts` (new), `src/utils/midiParser.ts`, `src/utils/trackChordRole.ts` (new), `src/utils/trackChordRoleTest.ts` (new), `src/utils/voiceLeading.ts` (new).

### Fixed
- **Erratic chord flicker under a moving melody.** Every micro-change to the set of ringing notes that produced a different name was pushed as its own event, and passing melody and ornament notes were counted as chord tones. A sustained A-major triad with a flute line over it cycled through "A", "Dmaj9/A", "Amaj7", "A6add9" and back; it now holds "A" — the melodic line is identified per note and set aside before the chord is read.
- **Chord changes inside a bar were averaged away.** The old detector summarised roughly a bar at a time, so three quick chords in one bar collapsed to one. It now marks a change wherever the set of held chord tones actually turns over, at any rate the music does.
- **A held pad hid the chords played over it.** When a string patch sustained one triad while the piano moved the harmony on top, the pad's energy swamped the piano and the whole passage read as one chord. A note that has been ringing for several bars now fades in weight, so the instrument actually playing the changes is heard.
...
- **Obscure chord names** like "Bsus24" or "Gb7b6". Detection weighed roughly a hundred `tonal` chord types equally and then tried to argue the strange ones down after the fact. Both the live display and the paused read-out now match against a hand-checked template set — every extended and altered quality a chart would actually use (maj7, 6/9, m7♭5, 9, 13, altered dominants…), and nothing that isn't a real chord name.
...
- **The chord name flashed up before you heard the chord.** It appeared on the beat line — the metronome position — so a chord struck late in the beat, or pushed onto the off-beat, showed its name up to a beat early (most of a second on a slow tune). The name now lands on the actual strike; tightly-quantised files are unaffected, expressive and syncopated playing now tracks the ear.
- **A rolled or arpeggiated chord chopped into a new name every beat.** On solo piano especially, a broken chord moved its lowest note each beat and the detector called each one a fresh chord — a single held bar could read as a dozen. It now recognises the same notes ringing underneath and holds one name until genuinely new notes are struck. (Sheet-music editions that print the sung melody into the piano staff can still over-colour a triad — being sharpened.)
- **A chord spelled out one note at a time — a guitar strum, a rolled piano chord, a broken bass line — could read as unrelated single notes instead of the chord it adds up to.** The chord-change logic was rebuilt around when notes are actually struck, gathering notes that arrive one after another into the one chord they spell, with the name landing right on the first note of the new chord rather than a beat later.
- **Whole instrument families (brass, reed, pipe, synth lead) were silently left out of the chord read.** A synth-brass part comping the actual changes is exactly as valid a chord source as a piano; a solo trumpet line is exactly as much a distraction as a solo flute line. Auto mode no longer filters by instrument name — only drums, the bass line, and true sound-effect patches are set aside — so which tracks carry the harmony is judged by what they actually play.
- **The chord sensitivity slider didn't change anything.** One of its two smoothing passes wasn't wired to it at all, so every setting produced an identical display. Both passes now respond to the slider.
- **A brief passing chord — struck for well under a beat, then straight back to what came before — could vanish from the display entirely**, on top of a separate issue where even a fully clean, confidently-read one was smoothed away at the normal sensitivity setting and only appeared with the slider turned up. Both fixed — a real passing chord now shows at the default setting.
- **A chord could appear before any note had actually sounded**, when the display was reading purely off a lone, unaccompanied melodic pickup note before the rest of the band comes in (an intro like Bruce Hornsby's "The Way It Is" showed a chord name across several bars of silence at the very start). It now waits for that note's own onset, same as any other chord.
- **Right-clicking the playback chord name could lock in — or open in the Chord Explorer — a completely different chord than the one on screen.** "Show on keyboard" was re-guessing the chord from every note sounding anywhere in the file at that instant, with no regard for which instruments actually carry the harmony; one extra note from an unrelated track playing at the same moment was enough to name something else entirely (a plain "A" could lock in as "Dmaj9/A"). It now always uses the exact chord already shown above the keyboard — only the voicing (register, bass note, inversion) is still read from the real notes at the playhead.
- **Changing the tempo and then scrubbing to a spot before pressing play threw the audio out of sync with the piano roll** — playback could sound like it was picking up from far earlier than the visible playhead, worse the further you'd scrubbed and the more the tempo had changed. A scheduling calculation was mixing a tempo-adjusted time with an unadjusted one; only lined up by coincidence when starting from the very beginning. Play now starts exactly where the playhead is, at any tempo.
...
- **The installer is now per-user again (`perMachine: false`).** 1.0.1's switch to a per-machine install is what broke "uninstall → delete everything": the uninstaller then ran elevated under the all-users shell context, where `$APPDATA` resolves to `C:\ProgramData`, so the cleanup deleted a folder that doesn't exist and the real `orfeo-prefs.json` (with the library-folder link) survived — a fresh reinstall reopened straight into the old library. Per-user install means no UAC prompt to install/uninstall/update, it lands in `%LOCALAPPDATA%\Programs\Orfeo`, and the uninstaller always runs as the right user so the data cleanup works. The cleanup itself was also hardened: it forces the invoking user's context, deletes the prefs/log files by name before the recursive delete (so a locked cache subfolder can't leave the library link behind), never touches data during an auto-update (`--updated`) or silent (`/S`) uninstall, and shows the exact path if the prefs file is somehow still locked rather than failing silently. **If you have an existing per-machine install in `C:\Program Files\Orfeo`, uninstall it once by hand.**
- **"Check for updates" is now split cleanly by build type.** `electron/main.ts` decides once whether this build does real auto-updates (`'auto'` — the installed Windows build only) or not (`'manual'` — portable, macOS, Linux, dev), and the renderer reads that on open. The installed build's button checks the GitHub feed and, if there's a newer release, starts the download automatically, then shows "Update ready — click to install" (or "Orfeo is up to date"). The portable build's button now has a fixed tooltip — "Check for new releases on GitHub" — and a single click opens the Releases page in the browser; it never shows a spurious "checking…"/round-trip state or needs a second click.
- **A failed update check no longer fills the screen.** electron-updater's errors are a full multi-line HTTP dump (every response header); the "no published release yet" case came through as one and the whole thing rendered as a giant tooltip that stuck around. Errors are now collapsed to a single short sentence, the tooltip is hard-capped in size, and the error state fades on its own. The GitHub `owner`/`repo` in the publish config were also corrected to the real casing (`Squarebow/Orfeo`).

### Changed
- `build/installer.nsh`, `docs/CHANGELOG.md`, `docs/CONTRIBUTING.md`, `docs/INSTALLATION.md`, `electron/main.ts`, `electron/preload.ts`, `package.json`, `src/components/ChordExplorer.tsx`, `src/components/Keyboard/Keyboard.tsx`, `src/components/LockedChordModal.tsx`, `src/components/LoopRegionStrip.tsx`, `src/components/ScaleExplorer.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/Tooltip.tsx`, `src/store/index.ts`, `src/types/index.ts`, `src/utils/midiParser.ts`.
...
- **Library auto-refreshes after every save.** A save from the Playback Editor, Note Editor, Mixer, tempo/key, or a foreign-format import now rescans the library folder immediately, expands the `Orfeo/` subfolder the new version landed in (previously it was created collapsed, so a just-saved file — and its embedded history in File info — was hidden until you found and opened that folder by hand), and briefly amber-highlights the new file. The highlight now matches against the actual rescanned row rather than the raw save path, so a drive-letter-case or separator mismatch can't leave the new version with no flash and no pinned bar. The manual refresh button and its blink-when-stale indicator stay as a fallback for whenever the automatic rescan can't run.
...
- **The Console Mixer now saves per-channel volume, pan, chorus, and reverb into the file.** Closing the mixer after changing any of them prompts **Save & Reload** (writes a new `_ORFEO_vN` version with a history entry and reloads it, same as the Playback Editor), Discard, or Cancel. The master strip and mute/solo stay session-only. The confirm button was renamed from "Save" to "Save & Reload" to match the Playback Editor's.
- **Uninstalling asks what to do with your Orfeo data — a three-way choice, not just Yes/No.** **Yes** deletes everything (preferences, library-folder link, cached soundfonts, demo files, and any edited MIDI saved without a library folder set); **No** moves those edited MIDI files to an `Orfeo Edited Files` folder on the Desktop and deletes the rest; **Cancel** keeps everything. Your library folder itself is never touched. A follow-up check retries and warns if the install folder couldn't be fully removed (a locked file or an antivirus scan) instead of silently reporting success.

### Fixed
- **Performance regression on the piano roll — slow file loads and high idle CPU.** Two hot paths were doing expensive work every playback tick: the live chord detector re-ran tonal.js's combinatorial chord search from scratch on every tick for both Classic and sustained modes (now memoized by exact sorted-MIDI content), and the chord-sequence derivation subscribed to `currentTime` and re-computed every frame (now keyed to the resolved note-on/off index). Plus draw-loop and re-render trims across `App`, `Keyboard`, `PianoRoll`, and `SettingsPanel`.
- **File info showed an empty history for a file you'd just saved.** The modal froze its target path at first mount, so renaming a file in the popup — or reopening File info on a different row while one was open — left the metadata, history, and version list describing the previous file. It now follows the file you actually clicked. (Combined with the auto-expand fix above, the Console Mixer's save history is now visible where you'd expect it.)
- **The Note Editor didn't fully close on a file switch.** With no unsaved edits it stayed open across opening a new file or Reset, still pointing at the old one: the right-click Assign Left/Right Hand options silently disappeared, and toggling hand-colour mode wiped the roll (the old file's solo-visibility snapshot applied positionally onto the new file's tracks). It now always closes and resets on every file switch, like its own ✕ button.
- **Scale Explorer — Circle of Fifths key-signature labels crowded the ring at some angles and floated free at others.** The ♯/♭ groups used a fixed anchor radius, but the label text is never rotated, so its width ate the gap near the horizontal midline and its height near the vertical one, both growing with symbol count. The radius is now solved per angle from the real glyph metrics, keeping a constant visual gap for 1 through 6 accidentals at every position.
- **Auto-Level on Load could distort loud passages.** The makeup-gain boost was applied *after* the compressor, so a loud hit landing right after a fast quiet run (a glissando into a downbeat) got amplified past what the compressor's attack had caught, clipping. The boost now sits before the compressor, its values are halved, and its time constants are stretched to multiple seconds so the gain only moves for genuinely sustained quiet passages, not brief ornamental runs. (Samples engine.)
- **Focus Mode** toggle behaviour corrected across the Tracks panel, Console Mixer master strip, and Settings.
...
- **Documentation restructured.** `docs/HOW_TO_USE.md` rewritten as a deep, navigable guide staged by mode of use (Play / Edit / Practice / Manage files / Extras) with per-feature tips and cross-links; `README.md` shortened to a feature overview that links out to the docs; `ARCHITECTURE.md`, `CONTRIBUTING.md`, `INSTALLATION.md`, and `SHORTCUTS.md` restyled to one house format and corrected against the shipping code — notably that the installer is per-machine and requires elevation now (`perMachine: true`), and that the audio section no longer describes the retired Tone.js / WAV-sample engine.

**Changed:** `package.json`, `electron/main.ts`, `electron.vite.config.ts`, `build/installer.nsh`, `build/license.txt` (new), `LICENSE.md` (renamed from `LICENSE`), `README.md`, `docs/CHANGELOG.md` (renamed), `docs/CONTRIBUTING.md` (renamed), `docs/ARCHITECTURE.md`, `docs/HOW_TO_USE.md`, `docs/INSTALLATION.md`, `docs/SHORTCUTS.md`, `THIRD_PARTY_LICENSES.md`, `scripts/generate-licenses.mjs` (new), `public/midi-editor.svg` (removed), `.gitignore`, `src/App.tsx`, `src/store/index.ts`, `src/components/FileInfoModal.tsx`, `src/components/Keyboard/Keyboard.tsx`, `src/components/MidiEditor/MidiEditor.tsx`, `src/components/Mixer/MixerConsole.tsx`, `src/components/Mixer/MasterStrip.tsx`, `src/components/NoteEditor/NoteEditorToolbar.tsx`, `src/components/PianoRoll/PianoRoll.tsx`, `src/components/ScaleExplorer.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/TrackPanel/TrackPanel.tsx`, `src/hooks/useAudioEngine.ts`, `src/hooks/useChordSequence.ts`, `src/hooks/useSamplesEngine.ts`, `src/utils/chordDetection.ts`, `src/utils/foreignFormatImport.ts`, `src/utils/midiMetadata.ts`, `src/utils/noteEditorState.ts`, `src/utils/tempoKeySave.ts`, `src/utils/velocityAnalysis.ts`.
...
### New
- **"Chords" settings section** — merged into "Notation & Chords" (same collapsible group as note-naming/accidentals, subsections kept in order). Two independent controls:
  - **Chord tracking** — what the live playback chord display and Lock-a-Chord follow: **Classic** (today's onset-clustering across all tracks, kept as-is), **General Harmony** (new — sustain-aware, tracks which notes are actually *ringing* at each instant rather than just which attack together, so a sustained chord under a moving melody stays correctly named instead of flickering between incidental melody/bass combinations), or **Follow Instrument** (General Harmony's algorithm scoped to one GM group — persisted — or one specific track in the current file — per-file, resets on load — with automatic fallback to General Harmony if the chosen group/track isn't present).
  - **Chord naming** — Abbreviations (`Bb(b5)/D`) or Symbols (`Bb(♭5)/D`, `+`/`°`/`ø`/`Δ` for aug/dim/half-dim/maj7), applied consistently everywhere a chord name appears: playback display, Lock-a-Chord, Chords Explorer, Scales Explorer.

### Fixed
- **Chord detection/naming was picking genuinely wrong or needlessly obscure names.** Two compounding bugs: `scoreChord` ranked candidates by raw string length, so a short exotic type (tonal.js's obscure quartal-harmony `"4"` type, e.g. `A4/C`) could beat a far more usable longer name (`D7sus4/C`); and tonal.js has no "omitted 3rd" chord types, so a common voicing missing its 3rd (e.g. C-D-G-B, a maj9 with no 3rd) detected as an unrelated, badly-altered chord from a different root (`Bmb6b9/C`) instead of anything C-rooted. Rewrote the scoring to weight actual complexity instead of length, and added a fallback that tries inserting a hypothetical 3rd and accepts it only when tonal agrees on the resulting root — verified against a real MIDI file end-to-end (`Bmb6b9/C`→`Cmaj9`, `A4/C`→`C6add9`, plus a same-notes-different-bass case found during testing, →`Cmaj9/B`).
- **A slash chord whose quality started with "M" lost its bass note** — e.g. `GM/D` (G major, 2nd inversion) rendered as `G( /D)`. The bass annotation was getting swallowed into the same-session naming-style formatter's parenthesization. Now split off before formatting and reattached after.
- **Lock-a-Chord / Chord Explorer preview audio played the wrong instrument** (e.g. alto sax instead of piano) once a real track in the loaded file happened to use the MIDI channel reserved for chord previews (channel 14 GM / 15 Samples) — its program-change setup only ever ran once per session, so a track using that channel during normal playback permanently reprogrammed the preview channel for the rest of the session. Now re-asserted before every preview note in both engines.
- **"Show on keyboard" (right-click the paused chord display) showed the chord in the wrong octave** — several octaves away from where it was actually playing — because it reconstructed a canonical fixed-octave voicing instead of using the real notes. Now uses the exact MIDI notes that were actually sounding, clamped to the current keyboard's range.
- **Closing Lock-a-Chord after Shift+clicking keys directly (not via right-click) left a stray amber focus-ring stuck on a key.** Piano keys are real focusable elements (for Enter/Space playability); clicking one gave it genuine browser focus, and the modal's focus-trap correctly restored focus to it on close per standard accessibility practice — triggering the app's `:focus-visible` amber outline rule with nothing to ever clear it. Mouse clicks on keys no longer take browser focus at all (`preventDefault` on mousedown); real Tab-driven keyboard navigation and its focus ring are unaffected.
- **MIDI Note Editor toolbar header** — title text ("MIDI NOTE EDITOR") was clipping, and the persistent centered hint wasn't reliably centered. Fixed-width side columns recalculated against the title's actual rendered width; disabled/inert info icon removed entirely rather than shown greyed-out.
- **Chords Explorer's fallback matching** — "Open in Chord Explorer" (from the playback chord-display's right-click menu) silently failed for any chord type outside the app's curated tile catalog (e.g. a major triad with a flattened 5th, `BbMb5`/`Bb(b5)`, only added a note off — no match), and searching by notes for the same chord returned nothing. Both now fall back to tonal.js's full chord dictionary when the curated catalog has no match, so any chord tonal can name is always reachable.
- **Settings panel: active/selected buttons and dropdowns rendered in monospace** (Sound Engine, Sound Fonts dropdown, hit-effect-pattern dropdown, the new Chords controls) instead of the UI's grotesk font — inconsistent with every other label in the panel.

### Improved
- **NoteEditorToolbar's hover-tooltip hint text** and the Snap/Quantize icons' tooltips rewritten for clarity — Snap and Quantize grid previously gave no indication of what they actually do (round a moved/resized/added note's timing to the grid; irrelevant while Snap is off), which read as "does nothing" since there's no visual grid overlay, only an effect at the moment you edit a note.

**Changed:** `CLAUDE.md` (new Versioning section, local-only), `package.json`, `src/components/ChordExplorer.tsx`, `src/components/Keyboard/Keyboard.tsx`, `src/components/MidiEditor/MidiEditor.tsx`, `src/components/NoteEditor/NoteEditorToolbar.tsx`, `src/components/ScaleExplorer.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/TrackPanel/TrackPanel.tsx`, `src/hooks/useAudioEngine.ts`, `src/hooks/useChordSequence.ts`, `src/hooks/useSamplesEngine.ts`, `src/store/index.ts`, `src/types/index.ts`, `src/utils/chordDetection.ts`.
...
### Fixed — real bugs found while doing the sweep, not just sizing
- **Library's pinned active-file row silently stopped sticking to the top of the list.** Root cause: `RowTooltip`'s wrapper `<div>` shrink-wraps to exactly its child's height, leaving `position:sticky` zero room to operate within its own containing block — the row just scrolled away with everything else instead of clamping in place, and the sticky "Folders" header beneath it (correctly stuck) was left with a gap showing the scrolling list peeking through above it. Confirmed with a live scroll test before and after the fix (the drift matched the broken math exactly, then clamped correctly at any scroll depth once fixed). Fixed by moving the sticky positioning onto `RowTooltip`'s own wrapper instead of the nested content `<div>`.
- **File Info modal's metadata rows silently exceeded their own `minHeight:24` floor** once the row content grew past what the row's padding+border budget allowed — a 1px-per-row creep that would have cascaded down through every row, Save/Cancel, and the History section. Caught via measurement (not visually) and fixed with a `lineHeight` pin sized to the tightest bordered row's real budget (11px), not a naive copy of the unbordered row's value.
- **MIDI Playback Editor** had four separate instances of the same class of bug — a relative `lineHeight` (`1.25`, `1.4`) or a default browser line-height silently growing past its element's real budget once the font size increased: the column-header row, two disclaimer rows, the Select-all/Clear-all buttons, and the Browse/Show-in-folder button (verified by comparing it against its never-touched sibling, the path-preview field, until both matched exactly).
- **Scale Explorer's chord-tile row was already exceeding its own `minHeight:44` floor before this pass touched it** (a pre-existing quirk of its two-line note+roman-numeral stack) — confirmed the true pre-edit height via a live DOM probe, then held the row to that same value instead of accidentally "fixing" a pre-existing 1px discrepancy that wasn't this pass's to fix. Its below-Circle-of-Fifths info row (root name/scale-quality/note-sequence) also needed pinning — it sits in normal flow, not a fixed box, so unpinned it would have grown the whole Circle-of-Fifths section and pushed every row below it down.
- **MIDI Note Editor's own header started truncating its title** ("MIDI NOTE EDIT…") the moment its font size grew to match every other modal — the panel was too narrow for the larger text. Measured the actual overflow instead of guessing: widened the toolbar and its header's two flanking grid columns (kept symmetric, so the centered hint text stays truly centered per this repo's own CSS Grid convention) until the title fit with zero clipping.

### Changed — sizing, weight, and family
- Every modal's header title now reads 14px (previously anywhere from 10-12px depending on which modal), the "global header" size applied consistently across MIDI Playback Editor, Console Mixer, MIDI Note Editor, Chord Explorer, Scale Explorer, File Info, and Lock-a-Chord.
- Library, Settings, Tracks, Keyboard, Chord/Scale Explorer, and the Console Mixer all had their smallest labels (8-9px) brought up to a 10-12px floor; several shared row-label constants got `fontWeight` eased from 700 to 600 per a follow-up review pass.
- **Filename text reverted from monospace back to the UI font (Grotesk)** in the TopBar and Library, after an earlier pass in this same audit had switched it to mono alongside the size bump — the user's actual preference was size-only there.
- New `FingerStepper` control (`< 4 >` toggle) replaces two full-width button rows for Settings' Max Fingers Left/Right, now one compact line under a single heading.
- New `labelOffset` prop on `MixerKnob` shifts a knob's label vertically with an equal-and-opposite compensating margin, so the widget's total footprint — and everything below it — never moves; used to pull the Chorus/Reverb/Tone/Compressor labels 10px closer to their knobs in both the channel strip and master strip.
- Settings' Visual Effects color swatch now defaults to a solid fill (the app's fixed piano left-hand blue) instead of a checkered "transparent" pattern when no color has been chosen yet; the Intensity/Spread/Threshold bloom sliders gained a "Drag to change" one-line tooltip (no evidence any existed before the 1.11.0 tooltip overhaul).
- TopBar playback-control tooltips (Go to start, Rewind 5s, Play/Pause, Forward 5s, Go to end) restyled to the app's one-line tooltip format; Loop's tooltip deliberately left in the richer two-part style. The tooltip on the loaded filename itself was removed entirely.

**Changed:** `src/components/{ChordExplorer,FileInfoModal,LockedChordModal,ScaleExplorer}.tsx`, `src/components/Keyboard/{Keyboard,KeyboardControls}.tsx`, `src/components/MidiEditor/MidiEditor.tsx`, `src/components/Mixer/{ChannelStrip,MasterStrip,MixerConsole,MixerKnob}.tsx`, `src/components/NoteEditor/NoteEditorToolbar.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/TrackPanel/TrackPanel.tsx`, `src/components/Transport/TopBar.tsx`, `src/components/VolumeKnob.tsx`.

## [1.11.0] — 16. 8. 2026 — App-wide tooltip system overhaul, Capella import support, demo-library sync fix

The Mixer's stylish tooltip styling (introduced in 1.10.0) is now the tooltip system for the entire app — every remaining native `title=` popup, replaced across three rounds of live-testing fixes. Alongside it: a third notation format (Capella) can now be imported and converted to MIDI, and the bundled demo library now actually reaches existing users when new demo files ship, not just fresh installs.

### New — app-wide tooltip system
- **A global boundary-clamping rule for every tooltip.** `TooltipBox` now measures its own rendered size and clamps/flips itself against the real window bounds before ever painting (two-pass: an invisible probe render to measure, then a `useLayoutEffect` correction before the browser's first paint, so there's no visible jump). This is what "the app's own rule" for tooltips near an edge means in practice — it's based on the anchor's actual on-screen position, so it automatically accounts for collapsed vs. expanded side panels, left-docked vs. right-docked rails, without any per-site special-casing. Fixes every reported edge-cropping case at once: collapsed Library/Settings/Tracks rails, the Orfeo logo, Chords, keyboard-size buttons, Scales, Presentation Mode, and more.
- **A second `oneLine` style**, alongside the original amber-heading + muted-description layout: plain white, normal-case, normal-weight text (can still wrap to two lines) with no heading row, for short self-explanatory hints that don't need the two-part structure. Applied to ~230 sites app-wide — Library (file/folder rows, star icons, folder actions), Settings (Sound Engine, Soundfonts, keyboard-size buttons, Orfeo/Manual/Download links), Tracks and the Console Mixer (Mute/Solo/Visible/Keyboard-lit), Chord Explorer, Scale Explorer, Lock-a-Chord, and TopBar. The original amber-heading style stays reserved for cases that genuinely need a title+explanation pair (Visual Effects scope/color, the Mixer's own knobs, MidiEditor's Save Tempo & Key toggle).
- **Tooltips removed entirely** wherever the control already explains itself elsewhere: Settings' toggle-icon rows (each has its own inline description already), Display System/Accidentals/keyboard-range buttons, Soundfonts' redundant per-item tooltip, the "Collapse all folders" header, close (X) buttons on Lock-a-Chord and the Playback Editor, root-row chord notes, and the note-count readout under the keyboard.
- **Right-click context menus unified into a shared `ContextMenu.tsx`** (`ContextMenu`, `ContextMenuItem`, `ContextMenuDivider`, `ContextMenuLabel`), replacing ~110 lines of copy-pasted `MENU_ITEM_STYLE` + per-button hover handlers across the Library's file- and folder-actions menus. Retheme to the same dark-panel/amber-border/glow language as the tooltips, instead of the old muted `--drag-handle-dot`/`--border-popover` borders.
- **New `useTooltip` hook** for elements that can't tolerate `Tooltip`'s wrapper `<div>` at all — e.g. a button living inside a `display:'contents'` CSS Grid row, where a wrapper element would either break the grid's direct-child passthrough or (if given `display:contents` itself) return a zero-sized rect at the window's origin from `getBoundingClientRect()`, putting the tooltip in the screen's top-left corner. `useTooltip` attaches hover-tracking and the anchor ref straight onto the caller's own element instead of introducing anything new into the DOM.

### Fixed — real bugs found while doing the sweep, not just styling
- **Library file/folder rows lost their full width and right-aligned favourite stars.** Root cause: `Tooltip`'s wrapper defaults to `display:'inline-flex'`, which does not stretch its child along the main axis even when the wrapper itself is given `width:'100%'` — the row inside kept shrinking to its own content width. This explained three symptoms at once: background painting only as wide as the filename, stars losing the extra space `MarqueeText`'s own `flex:1` used to push them into, and the marquee's overflow-detection measuring a collapsed width (so long filenames stopped scrolling). Fixed by pairing `display:'block'` with every `wrapperStyle={{width:'100%'}}` site; documented directly in `Tooltip.tsx`'s prop comment so it can't quietly recur.
- **Hovering a file's star icon showed two tooltips stacked at once** ("Add to favourites" and the row's own "Right-click for options") — the row's hover state stays true the whole time the pointer is anywhere inside it, including over a nested child, since `mouseenter`/`mouseleave` only fire on boundary crossings. Fixed with a small suppression-aware set (`RowTooltip`, `FavouriteStar`, `RowIconButton` in `SettingsPanel.tsx`) where nested icons report their own hover state up to the row via a ref-counted callback, so the row tooltip hides itself whenever a nested icon's tooltip is showing. Applied to all three places this pattern exists, including the folder row's Undo-all-moves and bulk-star icons, which had the identical latent bug before it was ever reported.
- **Soundfont Remove/Download tooltips rendered in the screen's top-left corner.** An earlier fix for this same section had tried `wrapperStyle={{display:'contents'}}` to preserve its CSS Grid row's direct-child passthrough — which traded one bug for another, since a `display:contents` element has no box of its own and its `getBoundingClientRect()` degenerates to a zero-sized rect at the origin. Replaced with the new `useTooltip` hook (see above), which never introduces a wrapper in the first place.
- **User-created Library folders had broken, overflowing tooltip text.** Folder rows had never been converted off the original elaborate "Expand folder — right-click for rename/delete/move options" wording, which is a full sentence rendered in the amber heading row's `whiteSpace:nowrap` styling (sized for short labels) — it visibly overflowed the tooltip box's own border. Recolored/reworded to match file rows ("Right-click for options", one-line).
- **TrackPanel's Mute/Solo/Visible/Keyboard-lit tooltips rendered as a tall block "growing upward in rows"** instead of a normal short tooltip. Not a positioning bug — several of the descriptions were 50-73 characters, wrapping to 3-4 lines inside the 200px-wide box. Shortened all 8 (both states × 4 buttons) to 1-2 lines. (Later converted to the `oneLine` style entirely, alongside the Console Mixer's equivalent icons, which live in a separate local `IBtn` implementation in `ChannelStrip.tsx`.)

### New — Capella (`.cap`) import support
- Added as a third foreign notation format alongside MusicXML and Guitar Pro, converted to Standard MIDI File bytes the same way (alphaTab's importer + `MidiFileGenerator`, disabled rendering engine). `ForeignFormat` is now `'musicxml' | 'guitarpro' | 'capella'`; every extension whitelist that gated this (drag-drop validation in `App.tsx`/`SettingsPanel.tsx`, the demo-folder sync filter, the recursive library scanner, the native "Open MIDI File" dialog filter) updated together, plus the `EmptyState` hint text. `pendingImportedFile.format` in the store was hardcoded to the old two-value union — caught by the type checker, now references `ForeignFormat` directly so this can't drift out of sync again.

### Fixed — bundled demo files now actually reach existing installs
- `ensureDemoFolder()` previously copied `public/demo/` into the user's library exactly once, ever, gated by a `.demo-installed` flag written on first launch — meaning new demo files added to `public/demo` in a later app version were invisible to anyone who'd already launched the app once, forever, even across updates. Rewritten to run on every launch instead, checking per-file whether it's already present at the destination rather than gating on a single flag: cheap (a handful of small files), self-healing (new files/subfolders in a later release reach existing installs on their next launch), and non-destructive (anything a user's deleted from their own `Demo/` folder stays deleted, since the sync is purely additive). Also now copies recursively, so subfolders in `public/demo/` (e.g. grouping MusicXML/Guitar Pro sample files separately from the root MIDI set) are preserved into the copy — the previous version silently skipped any directory since its flat `readdir` filtered on file extension.

**Changed:** `electron/main.ts`, `src/App.tsx`, `src/store/index.ts`, `src/utils/foreignFormatImport.ts`, `src/components/EmptyState.tsx`, `src/components/Tooltip.tsx`, `src/components/{ChordExplorer,LockedChordModal,LoopRegionStrip,ScaleExplorer,SpeedControl,VolumeKnob}.tsx`, `src/components/Keyboard/{Keyboard,KeyboardControls,FloatingKeyboard}.tsx`, `src/components/MidiEditor/MidiEditor.tsx`, `src/components/Mixer/ChannelStrip.tsx`, `src/components/PianoRoll/PianoRoll.tsx`, `src/components/SettingsPanel/SettingsPanel.tsx`, `src/components/TrackPanel/TrackPanel.tsx`, `src/components/Transport/TopBar.tsx`. **New:** `src/components/ContextMenu.tsx`.
...
### New — Master compressor/limiter
- **Real `DynamicsCompressorNode`, inserted last in the Samples-engine chain** (`gain → tone filter → compressor → destination`), catching whatever the gain and tone stages produce. Five presets (Safety / Gentle / Medium / Firm / Limiter) each bundle threshold, ratio, attack, and knee together rather than sweeping just one parameter — a fixed threshold with only ratio changing either barely engages at the gentle end or over-triggers at the aggressive end, so the presets wouldn't have sounded distinct from each other. Release is fixed at 250ms across all five; attack is what actually shifts the felt character from "gentle glue" to "hard limiter." On by default at the Gentle preset — this directly addresses a real gain-staging bug found earlier (`SAMPLES_BOOST × masterVolume` can exceed unity with zero limiting in the chain, causing clipping at default settings) rather than leaving it opt-in.
- **"Off" is a real transparent state** (ratio 1:1, threshold 0dB — mathematically a no-op) instead of disconnecting/reconnecting the audio graph on every toggle.
- **Master strip redesign** to fit the new control: the three global-action icons (mute-all/waterfall-all/keyboard-all) and the mute-filter button merged into one row to free up space without growing the strip; a new Tone+Compressor row puts the compressor's preset knob opposite Tone, with a bare on/off icon (amber = on, white = off, no button chrome) centered between them via CSS Grid — this repo's own layout convention (`minmax(0,1fr) auto minmax(0,1fr)`, documented in `CLAUDE.md`) rather than flex+space-between, so the toggle can't drift off-center if the two side knobs' widths ever diverge again. Strip widened 160px → 184px to fit.
- **Compressor icon** (`CompressorIcon.tsx`) restyled from a supplied reference SVG to match every other Orfeo icon's `currentColor` stroke pattern.
- **Preset knob has no static radial labels.** An early version placed ratio numbers around the knob, but those made its footprint wider than a plain knob and threw off its right-alignment against Reverb above it — removed in favor of a dynamic tooltip: hovering a tick shows that preset's info regardless of what's currently selected, hovering the knob's own center shows whichever preset is actually active.
- `getMasterCompReduction()` exported from the audio engine (live gain-reduction in dB, unsmoothed) — infrastructure for a planned visual-only "the master/track faders visibly dip while the limiter is working" treatment. Not wired to the UI yet.

### New — stylish tooltip system for the Mixer
- **`Tooltip.tsx`** — dark background, amber border, amber title row + muted description row, portaled to `document.body`. Replaces native `title=` browser tooltips on every Mixer knob (Chorus, Reverb, Tone, Compressor, Master Volume) and icon button (mute/waterfall/keyboard-all, mute-filter, Bars/FFT toggle, drag-handle, M/S/Eye/Kbd per track).
- **Master Volume's tooltip is live**, not static: shows the current percentage on hover AND while actively dragging the knob (via a new `onDragChange` callback on `MixerKnob`), replacing the permanent `%` readout that used to sit below the knob. Uses a new `left` placement (anchors to the rect's vertical center, not its top edge) — the default `top` placement floated the tooltip far above the actual 202px-tall knob since it positions off the anchor's top edge, not wherever the knob's value/handle actually is.
- **Note found for later:** the PianoRoll already has its own cursor-following rich tooltip style (`PianoRoll.tsx:359-381` — different tokens, `borderRadius:4`, no bold/uppercase title) that predates this one. Migrating the Mixer's `Tooltip.tsx` to match it is a deliberately deferred follow-up, not done in this pass.

### Fixed — global focus-ring bug (not just the Mixer)
- **Clicking a button with the mouse no longer leaves a stale focus ring that a later, unrelated keypress lights back up.** `:focus-visible` is spec-correct but non-intuitive: it shows based on "is the page's input modality currently keyboard AND does this element currently have focus," not "did this keypress activate this element." A mouse click leaves a button focused (normal browser behavior); the next keypress anywhere — including Space for play/pause, a global shortcut here that doesn't even depend on focus — flips the modality flag and the stale-focused button's ring reappears for a keypress that had nothing to do with it. Fixed by blurring non-text controls immediately after a genuine mouse click (`event.detail === 0` distinguishes a real pointer click from a synthetic click that Enter/Space fires when activating a focused element via keyboard, so real keyboard activation keeps its ring as intended).
- **Root-cause bug found while shipping the above fix:** the blur handler's target check used `el instanceof HTMLElement`, which silently rejected every click landing on an icon — most buttons in this app render an SVG as their visible content, and `SVGElement` is not an instance of `HTMLElement` (separate DOM interfaces). The fix looked correct against a synthetic test that clicked the `<button>` element directly, but failed on every real click, which almost always lands on the icon inside it. Caught via a CDP-dispatched trusted click (not `element.dispatchEvent(...)`, which doesn't reproduce this) after a user report; fixed by checking `instanceof Element` instead.

### Fixed — Mixer layout
- **Compressor knob/toggle alignment.** The "COMPRESSOR" label (wider than the 52px knob) was warping its flex column's shrink-to-fit width, so `justifySelf:'end'` aligned the wider label's edge to the grid track instead of the knob's — visibly left of Reverb above it. Fixed by giving both Tone and Compressor a fixed 52×52 box with the label absolutely positioned below, out of flow. CDP-measured after the fix: both knobs share exact left/right edges with Chorus/Reverb, and the toggle sits at the exact horizontal midpoint.
- Chorus/Reverb knob-to-label gap doubled (3px → 7px, `MixerKnob`'s own internal spacing — applies to every knob app-wide) after it read as "dangerously close."
- Master Volume knob's tick strokes halved via a new per-instance `tickStrokeScale` prop (0.5 on this knob only — every other knob keeps the app-wide default) after the density of its 36 ticks made full-thickness strokes look heavy.

### New — Console Mixer header marquee
- The permanent header description ("Adjust track levels, apply effects and compress overall loudness") now marquee-scrolls on hover when a narrow modal (few tracks loaded) can't fit the full sentence, instead of ellipsis-truncating; when it fits, it stays centered and static exactly as before. Overflow is measured live via `ResizeObserver` against the actual rendered text, not assumed from a fixed width, since track count changes the modal's width at runtime. Respects `prefers-reduced-motion`.
- **Bug found and fixed while building it:** the overflow-measuring effect was keyed to the store's `mixerOpen` flag, but the component itself renders `null` until a separate `everOpened` flag flips true one render later — so the very first measurement ran against refs that didn't exist yet (silently bailing), and by the time the DOM existed, `mixerOpen` was no longer changing, so the effect never ran again. Net effect: overflow was permanently reported as `0`, and with `overflow:hidden` centering, actually-overflowing text clipped to a meaningless slice from the *middle* of the sentence rather than showing the start. Rekeyed to `everOpened`, the flag that actually gates the DOM's existence.

**Changed:** `src/App.tsx`, `src/components/Mixer/{ChannelStrip,MasterStrip,MixerConsole,MixerKnob}.tsx`, `src/hooks/useSamplesEngine.ts`, `src/index.css`, `src/store/index.ts`. **New:** `src/components/CompressorIcon.tsx`, `src/components/Tooltip.tsx`.
...
### Accessibility — the app's weakest audited dimension, now addressed end to end
- **Visible keyboard focus app-wide.** `button:focus{outline:none}` had no replacement anywhere in the app — a sighted keyboard-only user tabbing through the UI had no way to see where focus was. Added a visible `:focus-visible` ring (amber accent) that doesn't reappear on mouse clicks.
- **Keyboard equivalents for mouse-only controls.** Mixer knobs, the channel fader, and on-screen piano keys were `<svg>`/`<div>` drag targets with no `tabIndex` or `onKeyDown` — a keyboard-only user could not adjust any mixer control or play a note from the on-screen keyboard at all. Added `tabIndex` + arrow-key increment/decrement to knobs and the fader; `tabIndex`/Enter-Space to piano keys. Also added a keyboard path to the Chords/Scales explorer triggers and the chord-prompter toggle, which were still `<span>`/`<div onClick>` even after the rest of the keyboard batch landed.
- **Modal semantics, done correctly, not blanket.** Every true modal (Mixer Console, Note Editor, Playback Editor, Scale/Chord Explorer, Lock-a-Chord, File Info, Confirm dialogs) now gets `role="dialog"`, `aria-modal="true"`, and a real focus trap (`src/hooks/useFocusTrap.ts`). Persistent non-modal tool panels (Floating Keyboard, Note Editor toolbar) deliberately got `role="region"`/`role="toolbar"` instead — a focus trap on those would have prevented keyboard users from ever tabbing back out to the rest of the app while either was open. Two follow-up bugs surfaced and fixed during this pass: the Mixer's focus trap silently never attached (a one-render mount-timing gap between `mixerOpen` and the modal actually existing in the DOM), and removing a stray Escape guard broke Chord/Scale Explorer's Escape behavior in a different way than intended — both fixed with the guard's real reasoning documented in place this time.
- **Consistent Escape-to-dismiss.** Scale Explorer and the Locked Chord modal had no Escape handler at all (confirmed by grep — the app's global handler was silently deferring to a component handler that didn't exist). All floating panels now dismiss on Escape. Locked Chord modal also previously double-fired global playback `stop()` on top of its own close (no store presence to exclude it from the global guard the way Chord/Scale Explorer already had) — added a `lockedChordModalOpen` store field for exactly that.
- **`prefers-reduced-motion` respected.** The TopBar's infinite loop-blink animation had zero motion-sensitivity guard anywhere in the codebase. Added a reduced-motion media query.
- **Contrast fixes.** `--text-track-meta`, `--text-track-count`, `--text-inactive`, and `--text-faint` all fell below WCAG AA's 4.5:1 minimum against their actual rendered backgrounds — every track row's channel/program/note-count text, and inactive icon states app-wide, were near-invisible for low-vision users. Raised all four to clear AA while preserving their relative dimness ordering.
- **ARIA on custom dropdowns/menus.** Settings' dropdown, the quantize-grid popover, Library's file/folder context menus, and the Piano Roll's right-click menu had no `role="listbox"/"menu"`, no `aria-expanded` — screen readers announced nothing beyond "button, unlabeled state." All four now carry proper roles.
- **Hand assignment (LH/RH) is no longer color-only during playback.** This is the app's stated pedagogical differentiator (which hand plays a note), and during normal falling-note playback it relied solely on `--hand-lh`/`--hand-rh` hue — a colorblind user lost the app's core teaching signal exactly when it mattered most. Added a non-color backup: a small L/R glyph on lit keyboard keys, and a thin left/right edge accent on hand-colored falling notes in the Piano Roll (shape/position, not hue, so it survives color-vision deficiency).
- **Settings form inputs now have a programmatic label association**, not DOM-proximity-only. `OptionRow` (used by nearly every Settings control) is wired as `role="group"` + `aria-labelledby` via `useId()` — a real `<label htmlFor>` wasn't feasible since its `children` is an opaque per-call-site control, not always one labelable element.

### Performance
- **Fader-drag stutter, root-caused and fixed everywhere it existed.** `TrackPanel.tsx`, `MidiEditor.tsx`, and `NoteEditorToolbar.tsx` all subscribed to the entire `tracks` array, which gets a new reference on every fader/pan/chorus/reverb drag tick from the Mixer — forcing a full re-render of whichever of those three happened to be open, dozens of times a second, on every drag. All three now subscribe to a narrow display-relevant signature string instead (the same pattern already proven in `ChannelStrip`/`MasterStrip`/`MixerConsole`), so unrelated drags no longer cascade into panels that don't even display the fields that changed.
- **Mixer VU-meter animation loops now stop when the Mixer is closed.** `ChannelStrip`/`MasterStrip` stay mounted (`display:none`) after first open so their internal state survives hide/show, but their `requestAnimationFrame` decay loops kept running indefinitely regardless — gated both on real `mixerOpen` state.

### Responsive design — TopBar and Mixer Console at the documented 900×600 floor
- Mixer Console's modal width now caps to the viewport (`calc(100vw - 24px)`) instead of computing up to ~1218px unbounded at high channel counts, which could render clipped off-window at the app's own documented minimum width.
- TopBar's center transport block rebuilt as a real 3×`minmax(0,1fr)` CSS Grid, replacing flex+absolute centering that could overlap the side groups near the 900px floor — the exact anti-pattern this repo's own `CLAUDE.md` already documents from an earlier `ChordExplorer.tsx` incident. A live-CDP-tested follow-up fix also stopped the left/right side groups themselves from overflowing into the center column at that width.
- **TopBar's right-side button-clearance padding, fixed twice — the first pass solved the wrong problem.** The Windows caption-button overlay (`–□×`) reserves real horizontal space, and the app was carrying a flat `174px` guess as permanent right-padding across the bar's entire height to avoid it — a much larger gap than actually needed, and one that didn't scale with the user's real display DPI. First attempt measured the real reserved width live via `navigator.windowControlsOverlay` (down to 137px on a 1920px-wide, 100%-scale display) but also bottom-anchored the right icon group below the buttons' 40px-tall zone to tighten the gap further — which fixed the padding but broke vertical alignment with the left/center groups (live user feedback caught this, not testing). Corrected: the group stays vertically centered like its siblings, and the live-measured inset moved to the group's own `paddingRight` instead of the row's shared padding, so its content's right edge lands exactly at the real button boundary — CDP-verified, zero overlap, no vertical trade-off. A genuinely clipped Volume-knob label (cut off 2.5px early by `overflowY:hidden`, found while measuring the above) was fixed in the same pass.

### Typography — tokenized, then self-hosted
- **Every hardcoded `'Inter'`/`'JetBrains Mono'` literal — 235 of them across 24 files — replaced with `var(--font-ui)`/`var(--font-mono)`.** Zero visual change; this was pure infrastructure so a future font swap is a two-line edit instead of a 235-site hunt. `--font-logo` is a separate token for the ORFEO wordmark's own distinct fallback stack, independently swappable from the UI font. Canvas2D `ctx.font` strings (the Piano Roll's falling-note overlay) can't resolve `var()`, so those plug into the file's existing resolve-once-from-CSS pattern (`resolvePianoRollColorsFromCSS`, already used there for PixiJS/Canvas colors) instead of duplicating the font names as separate literals.
- **Google Fonts CDN dependency removed entirely.** `PRODUCT.md` states this app is local-first with no network dependency for core playback; the UI font was quietly violating that on every launch. Downloaded the same files Google was serving (both are variable fonts, so only 4 physical `.woff2` files total cover the whole weight range) and committed them to `public/fonts/`, declared via local `@font-face` blocks. Verified against an actual `electron-vite build` output, not just the dev server — production Electron loads via `file://`, where absolute asset paths can resolve incorrectly; confirmed the build correctly rewrites them to relative paths that survive it.
- **Fixed a real, unrelated bug found while doing this**: weight 700 was never loaded for either font (only 400/500/600 for Inter, 400/600 for JetBrains Mono) despite being the single most-used weight app-wide — 69 uses across BPM/key numbers, chord names, mute/solo labels, and the logo. All of those were rendering as Chromium's synthetic/faux bold this whole time, not the fonts' real bold faces. Both now load 400/500/600/700 (Inter) and 400/600/700 (JetBrains Mono).

### Also fixed
- Drop-error toast (shown for unsupported file types) now has click-to-dismiss alongside its 2.5s auto-timeout.
- `SettingsPanel.tsx`'s hard-coded color literals (including one red encoded two different ways in the same file) tokenized; a leftover diagnostic `console.log` removed from the Mixer's close handler.
- The disabled "coming soon" practice-mode icon in the Track Panel now has a dashed outline and reduced opacity, so it reads as unavailable at rest instead of only on hover.
- A redundant `onClick` on the Track Panel's collapsed-group color-swatch row (a "visual legend only" element, per its own comment) removed — it duplicated the adjacent chevron button's toggle and falsely implied the swatches themselves were clickable.
- `MidiEditor.tsx`'s duplicated, undocumented `LOW_CONF` hex literal tokenized to `--status-low-confidence`.

**New:** `src/hooks/useFocusTrap.ts`, `public/fonts/Inter-Variable-{latin,latin-ext}.woff2`, `public/fonts/JetBrainsMono-Variable-{latin,latin-ext}.woff2`. **Changed:** most of `src/components/` (accessibility/perf/token passes touched nearly every floating panel and the Mixer/TrackPanel/Keyboard/PianoRoll/TopBar/SettingsPanel families), `src/store/index.ts`, `src/index.css`.
